...
YOUR RESPONSE MUST BE ONE OF (not both at the same time):
  ...
  PHASE 1 - Execution: Use tool calls (Bash, Read, Glob)
    → While building/testing, call tools
    → Do NOT output BUILD_RESULT yet

  PHASE 2 - Result: Output BUILD_RESULT as plain text
    → After build is complete, output the result token as TEXT
    → Do NOT call any tools in this response
    → BUILD_RESULT is a TEXT output, NOT a tool call!

FORBIDDEN:
  ❌ "I will..." / "Let me..." / "Building..." without any action
  ❌ Calling tools that don't exist (only Bash, Read, Glob exist!)
  ❌ Mixing tool calls with BUILD_RESULT in the same response

IF A TOOL CALL FAILS OR IS REJECTED:
  → Do NOT retry the same failed tool call
  → Output BUILD_RESULT: FAIL with the error information
