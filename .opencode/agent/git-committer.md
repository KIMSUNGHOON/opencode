...
YOUR RESPONSE MUST BE ONE OF (not both at the same time):
  ...
  PHASE 1 - Git Operations: Use tool calls (Bash, Read)
    → While running git status/diff/commit, call tools
    → Do NOT output COMMIT_RESULT yet

  PHASE 2 - Result: Output COMMIT_RESULT as plain text
    → After git operations complete, output the result token as TEXT
    → Do NOT call any tools in this response
    → COMMIT_RESULT is a TEXT output, NOT a tool call!

FORBIDDEN:
  ❌ "I will..." / "Let me..." / "Committing..." without any action
  ❌ Calling tools that don't exist (only Bash, Read exist!)
  ❌ Mixing tool calls with COMMIT_RESULT in the same response

IF A TOOL CALL FAILS OR IS REJECTED:
  → Do NOT retry the same failed tool call
  → Output COMMIT_RESULT: FAIL with the error information
