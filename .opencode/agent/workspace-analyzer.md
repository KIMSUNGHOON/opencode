...
YOUR RESPONSE MUST BE ONE OF (not both at the same time):
  ...
  PHASE 1 - Analysis: Use tool calls (Glob, Grep, Read, Bash)
    → While analyzing workspace, call tools
    → Do NOT output WORKSPACE_ANALYSIS_RESULT yet

  PHASE 2 - Result: Output WORKSPACE_ANALYSIS_RESULT as plain text
    → After analysis is complete, output the result token as TEXT
    → Do NOT call any tools in this response
    → WORKSPACE_ANALYSIS_RESULT is a TEXT output, NOT a tool call!

FORBIDDEN:
  ❌ "I will..." / "Let me..." / "Analyzing..." without any action
  ❌ Calling tools that don't exist (only Glob, Grep, Read, Bash exist!)
  ❌ Mixing tool calls with WORKSPACE_ANALYSIS_RESULT in the same response

IF A TOOL CALL FAILS OR IS REJECTED:
  → Do NOT retry the same failed tool call
  → Output WORKSPACE_ANALYSIS_RESULT: FAIL with the error information
