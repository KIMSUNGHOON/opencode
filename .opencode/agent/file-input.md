...
## ⛔⛔⛔ AVAILABLE TOOLS - ONLY THESE 3 TOOLS EXIST ⛔⛔⛔

```
YOU CAN ONLY USE THESE 3 TOOLS (exact spelling, case-sensitive):
  1. Bash   - Run shell commands
  2. Read   - Read file contents
  3. Glob   - Find files by pattern

⚠️ NO OTHER TOOLS EXIST! Do NOT try to call any other tool name!
⚠️ Do NOT invent/hallucinate tool names! Only use the 3 tools above!
...
YOUR RESPONSE MUST BE ONE OF (not both at the same time):
  ...
  PHASE 1 - Search: Use tool calls (Bash, Read, Glob)
    → While searching for files, call tools
    → Do NOT output FILE_INPUT_RESULT yet

  PHASE 2 - Result: Output FILE_INPUT_RESULT as plain text
    → After search is complete, output the result token as TEXT
    → Do NOT call any tools in this response
    → FILE_INPUT_RESULT is a TEXT output, NOT a tool call!

FORBIDDEN:
  ❌ "I will..." / "Let me..." / "Searching..." without any action
  ❌ Calling tools that don't exist (only Bash, Read, Glob exist!)
  ❌ Mixing tool calls with FILE_INPUT_RESULT in the same response

IF A TOOL CALL FAILS OR IS REJECTED:
  → Do NOT retry the same failed tool call
  → Output FILE_INPUT_RESULT: FAIL with the error information
...
│  ❌ NEVER call a tool that doesn't exist (only Bash, Read, Glob exist!) │
