...
- **하이브리드 모델 전략** (단일 서버 + 요청별 Thinking 제어):
  ...
  - Note: 동일 모델을 하나의 SGLang 서버(port 8000)에서 서빙. workspace-analyzer는 deprecated (legacy fallback).
...
| 항목 | Thinking Mode | Instruct Mode |
|------|---------------|---------------|
| **모델** | Qwen3.5-122B-A10B-FP8 | Qwen3.5-122B-A10B-FP8 (동일 모델) |
| **서빙 엔진** | SGLang (port 8000) | SGLang (port 8000, 동일 서버) |
| **요청별 제어** | `chat_template_kwargs: {"enable_thinking": true}` | `chat_template_kwargs: {"enable_thinking": false}` |
| **Context Window** | 256K | 256K |
| **Output Limit** | 32K | 32K |
| **Reasoning** | Yes (thinking mode) | No |
| **Tool Calling** | Yes | Yes |
| **담당 Agent** | code-reviewer, quality-checker, summary-reporter | Orchestrator (code-qa), env-setup, git-input, file-input, pre-checker, code-fixer, build-tester, function-tester, git-committer, git-pusher |
...
**Instruct Mode** (Orchestrator, env-setup, git-input, file-input, pre-checker, code-fixer, build-tester, function-tester, git-committer, git-pusher):
```
Temperature: 0.7
TopP: 0.8
TopK: 20
...
- GPU: 8x H100 NVL 96GB (또는 동급) — TP=8 단일 서버
- VRAM: 모델 + KV cache 수용 가능 용량
...
**단일 서버 — SGLang (포트 8000), Thinking/Instruct 모드 요청별 제어:**

```bash
# Qwen3.5-122B-A10B-FP8 단일 서버 서빙 (하이브리드 모드)
...
> **Note**: Thinking/Instruct 모드는 `chat_template_kwargs`의 `enable_thinking` 플래그로 요청별 제어됩니다. `opencode.jsonc`의 provider 설정에서 구성되므로 별도 서버가 필요 없습니다.
...
```json
{
  ...
  // 단일 SGLang 서버 (port 8000), 요청별 thinking 제어
  "provider": {
    "qwen": {
...
| code-fixer | Yes | Yes |
| quality-checker | Yes | Yes |
| build-tester | Yes | Yes |
| function-tester | Yes | Yes |
| git-committer | Yes | No |
| summary-reporter | Yes | Yes |
| git-pusher | Yes | No |
...
```
WARNING: 변경 파일이 150개입니다. 분석에 시간이 오래 걸릴 수 있습니다.
```
...
|-------|:----:|:----:|:----:|:-----:|:----:|:----:|----------|
| **workspace-analyzer** | Yes | Yes | No | No | Yes | Yes | 워크스페이스 분석 |
| env-setup | Yes | Yes | No | No | Yes | Yes | 환경 감지 |
| git-input | Yes | Yes | No | No | Yes | No | Git 파싱 |
| pre-checker | Yes | Yes | No | No | Yes | Yes | Lint/Format |
| **code-reviewer** | **No** | Yes | No | No | **No** | **No** | 코드 분석 |
| code-fixer | Yes | Yes | Yes | Yes | Yes | Yes | 코드 수정 |
| quality-checker | Yes | Yes | No | No | Yes | Yes | 품질 검사 |
| build-tester | Yes | Yes | No | No | Yes | No | 빌드 테스트 |
| function-tester | Yes | Yes | No | No | Yes | Yes | 기능 테스트 |
| git-committer | Yes | Yes | No | No | No | No | Git 커밋 |
| summary-reporter | Yes | Yes | No | No | No | No | 리포트 생성 |
...
|--------|------|---------|
| `/code-qa` | 전체 워크플로우 | 선택 |
| `/code-qa --files <경로>` | 전체 워크플로우 (Non-Git) | No |
| `/analyze` | 워크스페이스 분석 및 캐시 생성 | No |
| `/env` | 환경 설정만 | No |
| `/lint` | Lint/Format만 | No |
| `/review` | 코드 리뷰만 | 선택 |
| `/fix` | 코드 수정만 | No |
| `/quality` | 품질 검사만 | No |
| `/build` | 빌드 테스트만 | No |
...
- [아키텍처](./02-architecture.kr.md) - 전체 워크플로우 다이어그램
- [환경 설정](./03-environment-setup.kr.md) - 환경 설정 상세
- [워크스페이스 분석](./04-workspace-analysis.kr.md) - 워크스페이스 분석 워크플로우 설계
