> **Note**: This document references `sglang/gpt-oss-120b` model IDs which are outdated. The current system uses a single model: `glm/GLM-4.7-FP8` (355B MoE with Interleaved Thinking). See [14-code-qa-v4-quick-start.md](./14-code-qa-v4-quick-start.md) for current configuration. The agent creation patterns and frameworks described here remain valid.
