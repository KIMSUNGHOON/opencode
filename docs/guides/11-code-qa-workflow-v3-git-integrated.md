...
| 7 | `summary-reporter` | **결과 리포트 생성** | ❌ (자동) |
| 8 | `git-pusher` | Push & PR | ✅ **필수** |
...
**목적:** QA 결과를 Markdown 형식으로 요약 리포트 생성
...
### 🔄 회귀 정보
- 회귀 횟수: 1/3
- 원인: Quality Check 첫 시도 68%

### ➡️ 다음 단계
- [ ] Push: `git push origin feature/my-feature --force-with-lease`
- [ ] PR 생성: base=main, head=feature/my-feature

---
*Generated by Code QA Workflow v3*
```
...
**다음 단계:** Summary Report 생성
```

### 4.3 Summary Reporter Agent

`.opencode/agent/summary-reporter.md`:

```markdown
---
description: QA 결과 요약 리포트 생성 전문가
mode: subagent
model: vllm/gpt-oss-120b
color: "#16A085"
permission:
...
리포트를 Markdown 코드 블록으로 사용자에게 출력합니다.
```

### 4.4 Git Pusher Agent
